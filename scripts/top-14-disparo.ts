...
function blocoLead(r: Row, a: Analise): string[] {
  ...
  lines.push('')
  lines.push(`#### D-1 — Pré-engajamento Instagram (24h ANTES da Msg 1)`)
  lines.push(`*${a.pre_engajamento_ig}*`)
  lines.push('')
  lines.push(`#### D+0 — Msg 1 — Abertura cirúrgica (timeline hook + <80 palavras)`)
  lines.push('```')
  lines.push(a.abertura)
  lines.push('```')
  lines.push('')
  lines.push(`#### D+3 — Follow-up (se não respondeu — pivô diferente)`)
  lines.push('```')
  lines.push(a.followup_d3)
  lines.push('```')
  lines.push('')
  lines.push(`#### D+7 — Breakup message (última tentativa)`)
  lines.push('```')
  lines.push(a.followup_d7)
  lines.push('```')
  lines.push('')
